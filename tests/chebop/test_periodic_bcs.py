...
@pytest.mark.skip(reason="Eigenvalue computation with periodic BCs is very slow (516s) - needs optimization")
class TestPeriodicEigenvalues:
    ...
    def test_periodic_eigenvalues_second_order(self):
        ...
        NOTE: Skipped due to excessive runtime (>8 minutes). Eigenvalue
        computation with periodic BCs needs optimization.
        """
        N = chebop([0, 2*np.pi])
        N.op = lambda u: -u.diff(2)
        N.bc = 'periodic'
        ...
        for i in range(len(eigfuns)):
            for j in range(i+1, len(eigfuns)):
                ...
                if np.abs(eigvals[i] - eigvals[j]) > 0.1:
                    assert np.abs(inner_prod) < 1e-8, \
                        f"<φ_{i}, φ_{j}> = {inner_prod}, should be ~0"
...
class TestPeriodicAdvancedProblems:
    ...
    @pytest.mark.xfail(reason="Coupled systems with periodic BCs need special handling - under-constrained problem")
    def test_periodic_systems(self):
        ...
        N.bc = 'periodic'
        N.rhs = [chebfun(lambda x: 0*x, [0, 2*np.pi]),
                 chebfun(lambda x: 0*x, [0, 2*np.pi])]
...
class TestPeriodicRobustness:
    ...
    @pytest.mark.slow
    @pytest.mark.skip(reason="Periodic adaptive refinement is very slow (163s) due to RHS compatibility check - problem is ill-posed")
    def test_periodic_adaptive_refinement(self):
        """Test that periodic solver uses adaptive refinement correctly.

        NOTE: This test is skipped because np.abs(np.sin(3*x)) has non-zero mean over [0,2π],
        violating the periodic compatibility condition. The problem is mathematically ill-posed.
        """
        N = chebop([0, 2*np.pi])
        N.op = lambda u: u.diff(2)
        N.bc = 'periodic'
        ...
        # Now use less smooth forcing
        N.rhs = chebfun(lambda x: np.abs(np.sin(3*x)), [0, 2*np.pi])
        u_rough = N.solve()

...
class TestEmptyTrigtechOperations:
    """Tests specifically for empty Trigtech edge cases to hit lines 855, 881, 929."""

    def test_empty_sum_returns_zero_line_855(self):
        """Specifically test line 855: if self.size == 0: return 0.0"""
        from chebpy.trigtech import Trigtech
    ...
    def test_empty_cumsum_returns_copy_line_881(self):
        """Specifically test line 881: if n == 0: return self.copy()"""
        from chebpy.trigtech import Trigtech
        empty = Trigtech.initempty()
        assert empty.size == 0  # Verify it's empty
        result = empty.cumsum()
    ...
    def test_empty_diff_returns_copy_line_929(self):
        """Specifically test line 929: if m == 0: return self.copy()"""
        from chebpy.trigtech import Trigtech
        empty = Trigtech.initempty()
        assert empty.size == 0  # Verify it's empty
        result = empty.diff()

...
class TestHighOrderEquations:
    ...
    def test_sixth_order_beam_equation(self):
        ...
        assert abs(u_mid - (-0.0000217014)) < 1e-7, \
            f"Expected u(0.5) ≈ -0.0000217014, got {u_mid}"
    ...
    def test_fourth_order_constant_coefficients(self):
        ...
        u_mid = u(np.array([0.5]))[0]
        assert abs(u_mid - 0.0041498738) < 1e-6, \
            f"Expected u(0.5) ≈ 0.0041498738, got {u_mid}"
...
class TestVariableCoefficientChallenges:
    ...
    def test_sign_changing_coefficient(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            has_singularity_warning = any(
                "changes sign" in msg or "TYPE-CHANGING" in msg
                for msg in warning_messages
            )
            assert has_singularity_warning, \
                "Expected warning about sign-changing coefficient"
    ...
    def test_nearly_singular_operator(self):
        ...
        assert 0.12 < max_u < 0.13, \
            f"Expected max|u| ≈ 0.125, got {max_u}"
...
class TestExtremeDomains:
    ...
    def test_very_small_domain(self):
        ...
        assert abs(u_mid - 0.5000000625) < 1e-6, \
            f"Expected u(0.0005) ≈ 0.5000000625, got {u_mid}"
    ...
    def test_very_large_domain(self):
        ...
        assert abs(u_mid) < 1e-10, \
            f"Expected u(50) ≈ 0, got {u_mid}"
...
class TestBoundaryconditionEdgeCases:
    ...
    def test_large_robin_coefficient(self):
        ...
        assert abs(u_left - (-5.0050050050e-04)) < 1e-6, \
            f"Expected u(0) ≈ -5.005e-04, got {u_left}"
    ...
    def test_nearly_zero_rhs(self):
        ...
        assert max_u < 1e-12, \
            f"Expected max|u| near 0, got {max_u}"
...
class TestFourthOrderEigenvalues:
    ...
    def test_simply_supported_beam(self):
        ...
        for i, (ev, expected_ev) in enumerate(zip(eigenvals, expected)):
            ...
            assert rel_err < 1e-6, \
                f"Eigenvalue {i}: expected {expected_ev}, got {ev} (rel_err={rel_err})"
...
class TestOperatorAlgebra:
    ...
    def test_polynomial_operator_expression(self):
        ...
        # Check against MATLAB
        u_mid = u(np.array([0.5]))[0]
        assert abs(u_mid - (-0.1276259652)) < 1e-6, \
            f"Expected u(0.5) ≈ -0.1276259652, got {u_mid}"
...
class TestNumericalStability:
    ...
    def test_high_contrast_solution(self):
        ...
        assert np.max(rel_err) < 1e-6, \
            f"High contrast solution has large relative error: {np.max(rel_err)}"

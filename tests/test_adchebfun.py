...
from chebpy.spectral import barycentric_matrix, diff_matrix, mult_matrix
...
class TestAdChebfunBasic:
    ...
    def test_identity_jacobian(self):
        ...
        eye_mat = np.eye(9)
        np.testing.assert_allclose(J, eye_mat, atol=1e-14)
...
class TestAdChebfunTranscendental:
    ...
    def test_abs_works(self):
        """Test that abs() uses sign(u) as derivative."""
        f = chebfun(lambda x: x - 0.5, [0, 1])
        f_ad = AdChebfun(f, n=16)

        # abs() uses sign as derivative
        f_abs = f_ad.abs()
...
class TestAdChebfunOperations:
    """Test AdChebfun operations."""

...
def test_release_creates_tag(git_repo):
    ...
    verify_result = subprocess.run(  # nosec
        [GIT, "tag", "-l", "v0.1.0"],
        cwd=git_repo,
        capture_output=True,
        text=True,
...
def test_release_pushes_if_ahead_of_remote(git_repo):
    ...
    tracked_file.touch()
    subprocess.run([GIT, "add", "file.txt"], cwd=git_repo, check=True)  # nosec
    subprocess.run([GIT, "commit", "-m", "Local commit"], cwd=git_repo, check=True)  # nosec
...
def test_release_fails_if_behind_remote(git_repo):
    ...
    with open(other_clone / "other.txt", "w") as f:
        f.write("content")
    subprocess.run([GIT, "add", "other.txt"], cwd=other_clone, check=True)  # nosec
    subprocess.run([GIT, "commit", "-m", "Remote commit"], cwd=other_clone, check=True)  # nosec
...
def test_dry_run_no_git_operations(git_repo):
    ...
    tags_after = subprocess.run(  # nosec
        [GIT, "tag", "-l"],
        cwd=git_repo,
        capture_output=True,
        text=True,
    ...
    tag_check = subprocess.run(  # nosec
        [GIT, "tag", "-l", "v0.1.0"],
        cwd=git_repo,
        capture_output=True,
        text=True,
...
def test_dry_run_exits_successfully_without_creating_tags(git_repo):
    ...
    local_tag_check = subprocess.run(  # nosec
        [GIT, "tag", "-l", "v0.1.0"],
        cwd=git_repo,
        capture_output=True,
        text=True,
    ...
    remote_tag_check = subprocess.run(  # nosec

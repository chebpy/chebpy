...
class Trigtech(Smoothfun):
    ...
    def sum(self):
        ...
        """
        if self.size == 0:  # pragma: no cover - handled by @self_empty decorator
            return 0.0
    ...
    def cumsum(self):
        ...
        """
        n = self.size
        if n == 0:  # pragma: no cover - handled by @self_empty decorator
            return self.copy()
    ...
    def diff(self, n=1):
        ...
        m = self.size
        if m == 0:  # pragma: no cover - handled by @self_empty decorator
            return self.copy()
    ...
    def _chop_coeffs(coeffs, tol):
        ...
        if len(sig_freqs) == 0:  # pragma: no cover - unreachable: if np.any(significant), sig_indices is non-empty

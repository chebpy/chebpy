...
- **Owned here (edit freely):** `src/chebpy/**`, `tests/**`, `pyproject.toml`,
  `README.md`, `mkdocs.yml`, `docs/**` except `docs/mkdocs-base.yml`.
- **Rhiza-managed (do not edit in place):** `.rhiza/**`,
  `.github/workflows/rhiza_*.yml`, `.pre-commit-config.yaml`, `ruff.toml`,
  `pytest.ini`, `.bandit`, `.editorconfig`, `docs/mkdocs-base.yml`,
  `cliff.toml`, `.devcontainer/*`. Change these upstream in
  Rhiza (or via `.rhiza/template.yml`) and re-sync, rather than editing the
  synced artifact.
- **The root `Makefile` is the exception:** it is template-owned but 3-way merged,
  and exists to carry local overrides — see `MKDOCS_EXTRA_PACKAGES` and
  `COVERAGE_FAIL_UNDER` in it. Set repo-specific `make` variables there (or in an
  uncommitted `local.mk`); keep everything else in `.rhiza/make.d/**`.

The `files:` block of `.rhiza/template.lock` is authoritative when this list and the
lock disagree — the lock is regenerated by every sync, this list is not.
